...
namespace GBH
{
    public class ConVar
    {
        ...
        private static ConVarFlags _modifiedFlag;
        ...
        {
            if ((_modifiedFlag & ConVarFlags.Archived) != 0)
            {
                WriteConfiguration();

                _modifiedFlag &= ~ConVarFlags.Archived;
            }
        }
        ...
        {
            // TODO: fs needs write
            using (StreamWriter file = File.CreateText("config.cfg"))
            {
                file.WriteLine("// generated by something, please do modify");

                file.Write(
                    _convars.Select(entry => entry.Value)
                            .Where(convar => convar.TestFlag(ConVarFlags.Archived))
                            .Select(convar => new { Name = convar.Name.Replace("\"", "\\\""), Value = convar.GetValue<string>().Replace("\"", "\\\"") })
                            .Aggregate("", (str, convar) => $"{str}seta \"{convar.Name}\" \"{convar.Value}\"\r\n")
                );
            }
        }
        #endregion
